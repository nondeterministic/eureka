...
void Console::print_line(Type* font, std::string s)
{
	...
	for (unsigned i = 0; i < s.length(); i++, j++) {
		...
		int y = get_surface()->h - font->char_height() - offset;

/*
		// Check if we have to blit and write into the next line
		if ( s[i] == '\n' ||
				// Automatic line break at long words:
				x + (get_word(s, i).length() * font->char_width()) >= (get_surface()->w - 1 - font->char_width()) )
		{
			j = 0;
			x = offset;
...
*/
		font->printch(get_surface(), s[i], x, y);
...
void Console::animate_cursor(Type* font, int x, int y, int offset)
{
	...
	if (pre_cursor_x != cursor_x || pre_cursor_y != cursor_y) {
		font->printch(get_surface(), blank_icon,
					  (pre_cursor_x * font->char_width()) + (offset),
					  (pre_cursor_y * (font->char_height() + offset)) + (get_surface()->h - font->char_height() - offset));
	}
	...
	font->printch(get_surface(), cursor,
				  (cursor_x * font->char_width()) + (offset),
				  (cursor_y * (font->char_height() + offset)) + (get_surface()->h - font->char_height() - offset));
	SDLWindow::Instance().blit_console();
}
...
SDL_Surface* Console::get_surface()
{
  return SDLWindow::Instance().get_console_SDL_surface();
}
