...
int Charset::load_charset() {
  std::string icon_path = (std::string)DATADIR + "/simplicissimus/data/charset.png";
  ...
  if (!(_ptr_charset_surf = IMG_Load(icon_path.c_str()))) {
    std::cerr << "Couldn't load charset: " << IMG_GetError() << std::endl;
    return -1;
  }
  ...
  for (int y = 0; y < _ptr_charset_surf->h; y += _h) {
    for (int x = 0; x < _ptr_charset_surf->w - 1; x += _w) {
      ...
      // std::cout << "x: " << x2 << ", y: " << y2 << ", ascii: " << ascii << "\n";
