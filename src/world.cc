...
{
	// Weapons
	if (luaL_dofile(L, ((string)DATADIR + "/simplicissimus/data/" + (string)WORLD_NAME + "/weapons/defs.lua").c_str())) {
		cerr << "Couldn't execute Lua file: " << lua_tostring(L, -1) << endl;
		exit(1);
	}

	for (boost::filesystem::directory_iterator itr(((string)DATADIR + "/simplicissimus/data/" + (string) WORLD_NAME + "/weapons"));
			...
			++itr)
	{
		...
		if (fname.compare("defs.lua") != 0 && fname.find(".lua") != string::npos) {
			if (luaL_dofile(L, ((string) DATADIR + "/simplicissimus/data/" + (string)WORLD_NAME + "/weapons/" + fname).c_str())) {
	...
	// Shields
	if (luaL_dofile(L, ((string) DATADIR + "/simplicissimus/data/" + (string)WORLD_NAME + "/shields/defs.lua").c_str())) {
		cerr << "Couldn't execute Lua file: " << lua_tostring(L, -1) << endl;
		exit(1);
	}

	for (boost::filesystem::directory_iterator itr(((string)DATADIR + "/simplicissimus/data/" + (string)WORLD_NAME + "/shields"));
			itr != boost::filesystem::directory_iterator();
			++itr)
	{
		const string fname = itr->path().filename().string();
		...
		if (fname.compare("defs.lua") != 0 && fname.find(".lua") != string::npos) {
			if (luaL_dofile(L, ((string) DATADIR + "/simplicissimus/data/" + (string) WORLD_NAME + "/shields/" + fname).c_str())) {
				cerr << "Couldn't execute Lua file " << fname << ": " << lua_tostring(L, -1) << endl;
				exit(1);
			}
		}
	...
	// Monsters
	if (luaL_dofile(L, ((string) DATADIR + "/simplicissimus/data/" + (string) WORLD_NAME + "/bestiary/defs.lua").c_str())) {
		cerr << "Couldn't execute Lua file: " << lua_tostring(L, -1) << endl;
		exit(1);
	}

	for (boost::filesystem::directory_iterator itr(((string) DATADIR + "/simplicissimus/data/" + (string) WORLD_NAME + "/bestiary"));
			itr != boost::filesystem::directory_iterator();
			++itr)
	{
		const string fname = itr->path().filename().string();
		...
		if (fname.compare("defs.lua") != 0 && fname.find(".lua") != string::npos) {
			if (luaL_dofile(L, ((string) DATADIR + "/simplicissimus/data/" + (string) WORLD_NAME + "/bestiary/" + fname).c_str())) {
				cerr << "Couldn't execute Lua file " << fname << ": " << lua_tostring(L, -1) << endl;
				exit(1);
			}
		}
