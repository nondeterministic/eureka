...
do
   ...
   function img_path()
      return simpl_datapath() .. "/bestiary/spider.png"
   end
   ...
   function fight()
      if (distance > 10) then
      	 simpl_printcon(string.format("A %s tries to ram its venomous fangs into %s but cannot reach.",
      				      get_name(), player_name), true)
      	 return
      end
