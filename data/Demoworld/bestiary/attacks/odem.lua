function odem(monster)
   ...
   if (number_of_attacked_players == 0) then
      simpl_printcon(string.format("A %s breathes its hot odem at your party, but missed.", monster.name), true)
      return
   end

   simpl_printcon(string.format("A %s breathes its hot odem at your party...", monster.name), true)
   for k, v in pairs(attacked_players) do
      r = simpl_rand(1, 20) - 10
      attack_successful = r < simpl_get_ac(k)
      
      if (attack_successful == true) then
         damage = simpl_rand(5, 15)
