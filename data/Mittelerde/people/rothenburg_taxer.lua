...
do
	...
	function otherwise(item)
		if (item == "tax" or item == "taxes") then
			...
			simpl_printcon("Dost though not agree, that one cannot run a kingdom without taxes?")
			answer = simpl_getcon()
			simpl_printcon(string.format("%s ", answer))
			if (answer == "y" or answer == "yes") then
			   ...
			   simpl_printcon("Would you be so kind to pay that now.")
			   pay_answer = simpl_getcon()
			   simpl_printcon(string.format("%s ", pay_answer))
			   if (pay_answer == "y" or pay_answer == "yes") then
